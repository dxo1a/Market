...
import { LoginRequest } from '@/proto/market'
...
function useUserForm() {
  ...
  if ('loginAfterRegister' in route.query)
    additionalInfo.value = 'Вы успешно зарегистрировались. Теперь вы можете войти в систему'
...
<template>
  <FormCard @submit="onSubmit">
    ...
    <template #default>
      ...
      <div
        v-if="additionalInfo"
        class="text-sm text-primary items-center gap-2 flex justify-center"
      >
    ...
    <template #footer>
      <div class="relative py-5">
        ...
        <span class="text-primary text-md absolute inset-0 m-auto px-4 block bg-card w-fit h-fit"
          >или</span
        >
      </div>
