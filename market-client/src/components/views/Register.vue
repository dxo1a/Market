...
import { RegisterRequest } from '@/proto/market'
...
function useUserForm() {
  ...
  function validate() {
    ...
    if (userData.username.length < 4)
      errorStack.push('Имя пользователя должно содержать минимум 4 символа')
    return errorStack
  }
...
<template>
  <FormCard @submit="onSubmit">
    ...
    <template #footer>
      ...
      <Button to="/login" class="w-fit m-auto" :variant="'outlined'"> Войти </Button>
    </template>
  </FormCard>
</template>
<style scoped></style>
